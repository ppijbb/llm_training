    ...
    def forward(self, hidden_states):
        ...
        for expert_idx in range(self.num_experts):
            ...
            if mask.any():
                ...
                weights = routing_probs[mask, expert_idx:expert_idx+1]
        ...
        # expert_assignments는 반드시 1차원이어야 함 (bincount 요구사항)
        selected_experts_for_callback = selected_experts.detach()
        # 차원 확인 및 1차원으로 변환
        if selected_experts_for_callback.dim() > 1:
            selected_experts_for_callback = selected_experts_for_callback.flatten()
        elif selected_experts_for_callback.dim() == 0:
            selected_experts_for_callback = selected_experts_for_callback.unsqueeze(0)
        # 음수 제거 및 long 타입으로 변환 (bincount 요구사항)
        selected_experts_for_callback = selected_experts_for_callback.clamp(min=0).long()
        
        # 기본 routing 정보 저장
        self.last_selected_experts = selected_experts_for_callback
...
class DummyMoEModel(nn.Module):
    ...
    def forward(self, input_ids, labels=None, **kwargs):
        ...
        for moe_layer in self.moe_layers:
            hidden_states = hidden_states + moe_layer(hidden_states)
...
def test_callback_with_dummy_model(use_deepspeed=True, use_accelerate=True):
    ...
    # 데이터셋
    train_dataset = create_dummy_dataset(num_samples=5000, seq_length=32)
